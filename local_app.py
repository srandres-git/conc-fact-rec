...
from config import EXPECTED_COLS, ENV_FILE_PATH
from conc import conciliar_local
from clean_data import read_excel_file
from utils import get_provs_from_dwh, load_env_vars, get_most_recent_file
...
if test_provs is None:
    ...
    print('✅ Conexión a SAP exitosa. Datos de proveedores obtenidos correctamente.')
    ...
    missing_paths = [key for key in ['fact_sat_path', 'fact_sap_path', 'box_path', 'cp_path', 'output_path'] if key not in env_vars]
    if len(missing_paths) > 0:
        ...
        fact_sap = read_excel_file(get_most_recent_file(env_vars['fact_sap_path'],'.xlsx'), session_name='fact_sap', expected_columns=EXPECTED_COLS['fact_sap'], header=9)
